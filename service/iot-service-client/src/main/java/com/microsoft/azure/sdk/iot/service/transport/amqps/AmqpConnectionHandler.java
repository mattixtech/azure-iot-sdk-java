...
public abstract class AmqpConnectionHandler extends ErrorLoggingBaseHandler
{
    ...
    public void close()
    {
        synchronized (stateChangeLock)
        {
            ...
            try
            {
                //when the callback for onReactorFinal comes, only then is it safe to free the reactor
    ...
    {
        log.trace("Reactor finalized, freeing reactor resources");
        this.reactor.free();
    ...
    public void onLinkLocalClose(Event event)
    {
        ...
        //Reactor should respond with onSessionLocalClose call, where we close the connection. Cannot close the connection here
        // because it can only be done when all of its sessions have closed
    ...
    public void onSessionLocalClose(Event event)
    {
        ...
        //Reactor should respond with onConnectionLocalClose call, where we close the reactor. Cannot close the reactor here
        // because it can only be done when all of its connections have closed
