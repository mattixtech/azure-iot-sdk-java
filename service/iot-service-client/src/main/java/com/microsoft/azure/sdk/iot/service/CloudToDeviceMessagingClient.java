 ...
 */
@Slf4j
public class CloudToDeviceMessagingClient
{
     ...
     * @param amqpSendHandlerMessageSentCallback TODO
     */
    public CloudToDeviceMessagingClient(String connectionString, IotHubServiceClientProtocol iotHubServiceClientProtocol)
    {
        Tools.throwIfNullOrEmpty(connectionString, "iotHubConnectionString cannot be null");
        ...
        {
            //No IO operation is occuring during parsing of the connection string, so the IOException is just confusing. Wrapping
        ...
        Tools.throwIfNullOrEmpty(this.hostName, "The provided connection string is missing a userName");

        this.amqpSendHandler = new AmqpSendHandler(this.hostName, this.userName, this.iotHubServiceClientProtocol);
    }
    ...
    {
        this.amqpSendHandler.open(new IotHubServiceSasToken(iotHubConnectionString, sasTokenExpiryTime).toString());
        log.info("File upload notification listener opened");
    }

    public void sendMessageAsync(Message message, String deviceId, MessageSentCallback messageSentCallback, Object context)
    {
        this.amqpSendHandler.queueMessage(message, deviceId, messageSentCallback, context);
    }

    public void sendMessageAsync(Message message, String deviceId, String moduleId, MessageSentCallback messageSentCallback, Object context)
    {
        this.amqpSendHandler.queueMessage(message, deviceId, moduleId, messageSentCallback, context);
    }
